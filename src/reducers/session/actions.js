...
export const updateUser = () => dispatch => {
  ...
  apiUpdateUser(currentUserInfo.id, userData)
    .then(response => {
      dispatch(notificationsActions.requestSuccess())
      dispatch({ type: Types.UPDATE_USER_SUCCESS, payload: { user: response.data } })
      dispatch(getUser())
    })
    .catch(error => {
      dispatch(notificationsActions.requestFail(error))
...
export const changePassword = () => dispatch => {
  ...
  if (passwords.confirmedPassword !== passwords.newPassword) {
    throwNotification('Password doesn\'t match. Please confirm your new password correctly')
  } else {
    dispatch(notificationsActions.requestStart())
    ...
    apiChangePassword(
      ...
      .then(response => {
        dispatch(notificationsActions.requestSuccess())
        dispatch({ type: Types.CHANGE_PASSWORD_SUCCESS, payload: { user: response.data } })
      })
      .catch(error => {
        dispatch(notificationsActions.requestFail(error))
      })
